...
describe('CALCULATE_DISTANCE', () => {
    ...
    describe('check the points-object ', () => {
        it('should throw error when the points are undifined', () => {
            expect(() => calculateDistance()).toThrow('point1 or point2 are undifined')
        })
        it('should throw error when the point1 or point2 are not a Pointobject', () => {
            expect(() => calculateDistance({ x: 4, y: 2 }, '2')).toThrow('point1 or point2 are not a pointObject')
            expect(() => calculateDistance(mockConstructorPoint(new Point(1, 1)), { x: 4, y: 2 })).toThrow('point1 or point2 are not a pointObject')
            expect(() => calculateDistance("45", mockConstructorPoint(new Point(1, 1)))).toThrow('point1 or point2 are not a pointObject')
            expect(() => calculateDistance(7, mockConstructorPoint(new Point(1, 1)))).toThrow('point1 or point2 are not a pointObject')
            expect(() => calculateDistance(["45", 56], mockConstructorPoint(new Point(1, 1)))).toThrow('point1 or point2 are not a pointObject')
        })
    })
})
...
describe('CALCULATE_JUNCTION_POINT', () => {
    ...
    describe('check the lines-object ', () => {
        it('should throw error when the lines are undifined', () => {
            expect(() => calculateJunctionPoint()).toThrow('line1 or line2 are undifined')
        })
        it('should throw error when the line1 or line2 are not a LineObject', () => {
            expect(() => calculateJunctionPoint(mockConstructorLine(new Line({ slope: 1, n: 8 })), '2')).toThrow('line1 or line2 are not a LineObject')
            expect(() => calculateJunctionPoint(2, mockConstructorLine(new Line({ slope: 1, n: 8 })))).toThrow('line1 or line2 are not a LineObject')
            expect(() => calculateJunctionPoint(2, [3, '6'])).toThrow('line1 or line2 are not a LineObject')
            expect(() => calculateJunctionPoint(mockConstructorLine(new Line({ slope: 1, n: 8 })), mockConstructorPoint(new Point({ slope: 1, n: 8 })))).toThrow('line1 or line2 are not a LineObject')
            expect(() => calculateJunctionPoint(mockConstructorLine(new Line({ slope: 1, n: 8 })), { slope: 1, n: 8 })).toThrow('line1 or line2 are not a LineObject')
        })
    })
})
...
describe('IS_POINT_ON_LINE', () => {
    ...
    describe('check the pointAndLine-objects ', () => {
        it('should throw error when the point or line are undifined', () => {
            expect(() => isPointOnLine()).toThrow('point or line are undifined')
        })
        it('should throw error when the line or point are not a LineObject or PointObject', () => {
            expect(() => isPointOnLine(mockConstructorLine(new Line({ slope: 4, n: 2 })), '2')).toThrow('line or point are not a lineObject or a pointObject')
            expect(() => isPointOnLine(2, mockConstructorLine(new Line({ slope: 4, n: 2 })))).toThrow('line or point are not a lineObject or a pointObject')
            expect(() => isPointOnLine(2, 3)).toThrow('line or point are not a lineObject or a pointObject')
        })
    })
})
