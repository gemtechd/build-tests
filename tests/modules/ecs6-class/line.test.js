...
describe("LINE", function () {
    ...
    describe("CALCULATE_N_OF_LINE_FUNCTION", function () {
        it("should return n of line", function () {
            const poix = new Point({ x: 3, y: 4 });
            const poiy = new Point({ x: 1, y: 3 });
            const li1 = new Line({ point1: poix, point2: poiy, slope: 0.5, n: 2.5 })
    ...
    describe("GET_POINT_BY_X", function () {
        ...
        it("should calculate the values of the missing data", function () {
            const poix = new Point({ x: 1, y: 5 });
            const poiy = new Point({ x: 5, y: 1 });
            let li1 = new Line({ point1: poix, point2: poiy, n: 7 })
            let li2 = new Line({ point1: poix, point2: poiy, slope: 7 })
            expect(li2.getPointByX(3)).toStrictEqual(new Point({ x: 3, y: 19 }))
            expect(li1.getPointByX(3)).toStrictEqual(new Point({ x: 3, y: 4 }))
        })
    })
    ...
    describe("EERORS", function () {
        ...
        it("should return error if argument is not number", function () {
            expect(function () { new Line({ point1: poix, point2: poix, n: "rtry", slope: 9 }) }).toThrow(new Error("argument n must be type number"))
            expect(function () { new Line({ point1: poix, point2: poix, n: true, slope: 9 }) }).toThrow(new Error("argument n must be type number"))
            expect(function () { new Line({ point1: poix, point2: poix, n: 8, slope: () => { } }) }).toThrow(new Error("argument slope must be type number"))
            expect(function () { new Line({ point1: poix, point2: poix, n: 8, slope: [1, 2] }) }).toThrow(new Error("argument slope must be type number"))
        })
