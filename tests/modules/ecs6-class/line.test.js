...
describe('CONSTRUCTOR', () => {
    ...
    it('should initialize point2, n and slope to default values when no parameters are provided', () => {
        ...
        expect(line1.point1.x).toBe(2);
        expect(line1.point1.y).toBe(5);
        expect(line1.point2.x).toBe(0);
        expect(line1.point2.y).toBe(0);
        expect(line1.n).toBe(undefined);
        expect(line1.slope).toBe(undefined);
    })

    it('should initialize point1, n and slope to default values when no parameters are provided', () => {

        const line2 = new Line({ point2: new Point({ x: 2, y: 5 }) });
        expect(line2.point1.x).toBe(0);
        expect(line2.point1.y).toBe(0);
        expect(line2.point2.x).toBe(2);
        expect(line2.point2.y).toBe(5);
    ...
    it('should initialize point1, point2 and n to default values when no parameters are provided', () => {
        ...
        expect(line3.point1.x).toBe(0);
        expect(line3.point1.y).toBe(0);
        expect(line3.point2.x).toBe(0);
        expect(line3.point2.y).toBe(0);
    ...
    it('should initialize point1,point2 and slope to default values when no parameters are provided', () => {
        ...
        expect(line4.point1.x).toBe(0);
        expect(line4.point1.y).toBe(0);
        expect(line4.point2.x).toBe(0);
        expect(line4.point2.y).toBe(0);
...
describe('CALCULATE_SLOPE', () => {
    ...
    it('should result in NaN for the same points (division by zero)', () => {
        const line = (new Line({ point1: mockConstructor(new Point({ x: 8, y: 5 })), point2: mockConstructor(new Point({ x: 8, y: 5 })) }));
    ...
    it('throw error for points with the same x-coordinate', () => {
        const line = mockConstructor(new Line({ point1: mockConstructor(new Point({ x: 5, y: 2 })), point2: mockConstructor(new Point({ x: 5, y: 7 })) }));
    ...
    it('should calculate slope as 0 for points with the same y-coordinate', () => {
        const line = mockConstructor(new Line({ point1: mockConstructor(new Point({ x: 1, y: 5 })), point2: mockConstructor(new Point({ x: 2, y: 5 })) }));
        line.calculateSlope();
    ...
    it('should calculate the slope correctly for different points with positive slope', () => {
        const line = mockConstructor(new Line({ point1: mockConstructor(new Point({ x: 1, y: 2 })), point2: mockConstructor(new Point({ x: 3, y: 8 })) }));
        line.calculateSlope();
        expect(line.slope).toEqual(3);
    });

    it('should calculate the slope correctly for different points with negative slope', () => {
        const line = mockConstructor(new Line({ point1: mockConstructor(new Point({ x: 4, y: 0 })), point2: mockConstructor(new Point({ x: 2, y: 6 })) }));
        line.calculateSlope();
...
describe('CALCULATENOFLINEFUNCTION', () => {
    ...
    it('calculates the correct n value ', () => {
        const line = mockConstructor(new Line({ point1: mockConstructor(new Point({ x: 2, y: 5 })), slope: 3 }));
...
describe('GET_POINT_ON_X_ASIS', () => {
    ...
    it('returns the correct point on the x-axis for the given slope and n value', () => {
        ...
        expect(line.getPointOnXAsis()).toEqual(mockConstructor(new Point({ x: 3, y: 0 })));
    })
...
describe('GET_POINT_ON_Y_ASIS', () => {
    ...
    it('returns the correct point on the y-axis for the given slope and n value', () => {
        ...
        expect(line.getPointOnYAsis()).toEqual(mockConstructor(new Point({ x: 0, y: 5 })));
...
describe('GET_POINT_BY_X', () => {
    ...
    it('should throw error when x is undefined', () => {
        const line = mockConstructor(new Line({ slope: 5, n: 7 }));
    ...
    it('getPointByX returns the correct Point for the given x value', () => {
        const line = mockConstructor(new Line({ slope: 5, n: 7 }));
        expect(line.getPointByX(4)).toEqual(mockConstructor(new Point({ x: 4, y: 27 })));
    })
...
describe('GET_POINT_BY_Y', () => {
    ...
    it('getPointByY returns the correct Point for the given y value', () => {
        const line = mockConstructor(new Line({ slope: 5, n: 5 }));
        expect(line.getPointByY(20)).toEqual(mockConstructor(new Point({ x: 3, y: 20 })));
    })
